...
namespace RBX_Alt_Manager
{
    public partial class AccountManager : Form
    {
        ...
        public static string CurrentJobId
        {
            ...
            {
                for (int i = 0; i < 4; i++)
                    if (Instance.SlotRadios[i].Checked)
                        return Instance.SlotJobIds[i].Text;
                return Instance.SlotJobIds[0].Text;
            }
        }
        public static IniSection SlotSettings;
        private RadioButton[] SlotRadios = new RadioButton[4];
        private TextBox[] SlotNames = new TextBox[4];
        private TextBox[] SlotPlaceIds = new TextBox[4];
        private TextBox[] SlotJobIds = new TextBox[4];
        private bool _syncingPlaceId = false;
        ...
        private static EventWaitHandle rbxSingletonEvent;
        ...
        public AccountManager()
        {
            ...
            SendMessage(SearchBar.Handle, EM_SETCUEBANNER, IntPtr.Zero, "Search accounts...");
        ...
        private void LoadAccounts(byte[] Hash = null)
        {
            ...
            // Fix duplicate BrowserTrackerIDs that cause AutoCloseLastProcess to kill the wrong instance
            var trackerIds = new HashSet<string>();
            foreach (Account account in AccountsList)
            {
                if (string.IsNullOrEmpty(account.BrowserTrackerID) || !trackerIds.Add(account.BrowserTrackerID))
                {
                    Random r = new Random(account.UserID.GetHashCode() ^ Environment.TickCount);
                    account.BrowserTrackerID = r.Next(100000, 175000).ToString() + r.Next(100000, 900000).ToString();
                    trackerIds.Add(account.BrowserTrackerID);
                }
            }
        ...
        public bool UpdateMultiRoblox()
        {
            ...
            if (Enabled && rbxMultiMutex == null)
            {
                for (int attempt = 1; attempt <= 3; attempt++)
                {
                    try
                    {
                        rbxMultiMutex = new Mutex(false, "ROBLOX_singletonMutex");
                ...
                if (rbxMultiMutex == null)
                    DebugLog("[MultiRoblox] FAILED to acquire mutex after 3 attempts");
                ...
                return rbxMultiMutex != null || rbxSingletonEvent != null;
            }
            else if (!Enabled && (rbxMultiMutex != null || rbxSingletonEvent != null))
            {
                if (rbxMultiMutex != null) { rbxMultiMutex.Close(); rbxMultiMutex = null; }
                if (rbxSingletonEvent != null) { rbxSingletonEvent.Close(); rbxSingletonEvent = null; }
            }
        ...
        private void JoinServer_Click(object sender, EventArgs e)
        {
            ...
            if (PlaceID.Text.Contains("privateServerLinkCode") && IDMatch.Success)
                SetSelectedSlotJobId(PlaceID.Text);
            ...
            string jobIdText = CurrentJobId;
            bool VIPServer = jobIdText.Length > 4 && jobIdText.Substring(0, 4) == "VIP:";
            ...
            string jobIdForJoin = VIPServer ? jobIdText.Substring(4) : jobIdText;
            ...
            new Thread(async () => // finally fixing an ancient bug in a dumb way, p.s. i do not condone this.
            {
                if (LaunchMultiple)
                {
                    ...
                    await LaunchAccounts(SelectedAccounts, PlaceId, jobIdForJoin, false, VIPServer);
                }
                else if (SelectedAccount != null)
                {
                    string res = await SelectedAccount.JoinServer(PlaceId, jobIdForJoin, false, VIPServer);
        ...
        private void SearchBar_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                AccountsView.ModelFilter = null;
            }
            else
            {
                AccountsView.ModelFilter = new BrightIdeasSoftware.ModelFilter(model =>
                {
                    Account account = model as Account;
                    if (account == null) return false;

                    return (account.Username ?? "").ToLower().Contains(searchText) ||
                           (account.Alias ?? "").ToLower().Contains(searchText) ||
        ...
        private void copyRbxplayerLinkToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (SelectedAccount.GetAuthTicket(out string Ticket))
            {
                ...
                bool HasJobId = string.IsNullOrEmpty(jobId);
                double LaunchTime = Math.Floor((DateTime.UtcNow - new DateTime(1970, 1, 1)).TotalSeconds * 1000);

                Random r = new Random();
                Clipboard.SetText(string.Format("<roblox-player://1/1+launchmode:play+gameinfo:{0}+launchtime:{4}+browsertrackerid:{5}+placelauncherurl:https://assetgame.roblox.com/game/PlaceLauncher.ashx?request=RequestGame{3}&placeId={1}{2}+robloxLocale:en_us+gameLocale:en_us>", Ticket, PlaceID.Text, HasJobId ? "" : ("&gameId=" + jobId), HasJobId ? "" : "Job", LaunchTime, r.Next(100000, 130000).ToString() + r.Next(100000, 900000).ToString()));
            }
        }
        ...
        private void SaveToAccount_Click(object sender, EventArgs e)
        {
            ...
            foreach (Account account in AccountsView.SelectedObjects)
            {
                ...
                if (string.IsNullOrEmpty(PlaceID.Text) && string.IsNullOrEmpty(jobId))
                {
                    account.RemoveField("SavedPlaceId");
                    account.RemoveField("SavedJobId");
                ...
                if (jobId.Contains("privateServerLinkCode") && Regex.IsMatch(jobId, @"\/games\/(\d+)\/"))
                    PlaceId = Regex.Match(jobId, @"\/games\/(\d+)\/").Groups[1].Value;

                account.SetField("SavedPlaceId", PlaceId);
                account.SetField("SavedJobId", jobId);
            }
        }
        ...
        {
            SlotSettings = IniSettings.Section("Slots");
            int sel = SlotSettings.Exists("Selected") ? SlotSettings.Get<int>("Selected") : 0;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                SlotRadios[i] = new RadioButton
                {
                    Location = new System.Drawing.Point(0, y + 2),
                    Size = new Size(16, 16),
                    Checked = (i == sel),
                    TabStop = false
                };
                SlotNames[i] = new TextBox
                {
                    Location = new System.Drawing.Point(19, y),
                    Size = new Size(55, 20),
                    Text = SlotSettings.Exists($"Name{i}") ? SlotSettings.Get($"Name{i}") : $"Server {i + 1}"
                };
                SlotPlaceIds[i] = new TextBox
                {
                    Location = new System.Drawing.Point(77, y),
                    Size = new Size(65, 20),
                    Text = SlotSettings.Exists($"Place{i}") ? SlotSettings.Get($"Place{i}") : ""
                };
                SlotJobIds[i] = new TextBox
                {
                    Location = new System.Drawing.Point(145, y),
                    Size = new Size(123, 20),
                    Text = SlotSettings.Exists($"Job{i}") ? SlotSettings.Get($"Job{i}") : ""
                };

                SlotNames[i].Enter += (s, e) => SlotRadios[idx].Checked = true;
                SlotPlaceIds[i].Enter += (s, e) => SlotRadios[idx].Checked = true;
                SlotJobIds[i].Enter += (s, e) => SlotRadios[idx].Checked = true;

                SlotRadios[i].CheckedChanged += (s, e) =>
                {
                    if (((RadioButton)s).Checked)
                    {
                        SlotSettings.Set("Selected", idx.ToString());
                        SyncPlaceIdFromSlot(idx);
                    }
                };

                SlotPlaceIds[i].TextChanged += (s, e) =>
                {
                    if (SlotRadios[idx].Checked)
                        SyncPlaceIdFromSlot(idx);
                };

                SlotPanel.Controls.Add(SlotRadios[i]);
                SlotPanel.Controls.Add(SlotNames[i]);
                SlotPanel.Controls.Add(SlotPlaceIds[i]);
                SlotPanel.Controls.Add(SlotJobIds[i]);
            }

            SyncPlaceIdFromSlot(sel);
        }

        private void SyncPlaceIdFromSlot(int slotIndex)
        {
            if (_syncingPlaceId) return;
            _syncingPlaceId = true;
            PlaceID.Text = SlotPlaceIds[slotIndex].Text;
            _syncingPlaceId = false;
        }
        ...
        private void SaveSlots()
        {
            for (int i = 0; i < 4; i++)
            {
                SlotSettings.Set($"Name{i}", SlotNames[i].Text);
                SlotSettings.Set($"Place{i}", SlotPlaceIds[i].Text);
                SlotSettings.Set($"Job{i}", SlotJobIds[i].Text);
            }
        }
        ...
        {
            for (int i = 0; i < 4; i++)
                if (SlotRadios[i].Checked) return i;
            return 0;
        }

        public void SetSelectedSlotJobId(string value)
        {
            SlotJobIds[GetSelectedSlotIndex()].Text = value;
        }

        public void SetSelectedSlotPlaceId(string value)
        {
            SlotPlaceIds[GetSelectedSlotIndex()].Text = value;
        }
    }
}